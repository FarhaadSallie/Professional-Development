...
def placeNewOrder(): #complete
    ...
    payload = '{"instrument": "BK-LOCKEDUSD","type":"limit", "side":"'+str(ActionBKUSD)+'","quantity":"'+str(Quantity)+'","limit_price":"'+str(BKLOCKEDUSD)+'","time_in_force":"gtd","expire_time":"'+str(milliseconds)+'"}'
    headers = {
    ...
    try:
        ...
        printError(e)
    print("BKUSDLocked",str(ActionBKUSD),"order made")
#excel import time
...
def fetchValues():
    try:
        ...
        BKLOCKEDUSD = format(sheet_obj['H22'].value,'.4f')
...
def printError(e):
    with open('Error.txt', 'a') as f:
        ...
        f.write('\n BKLockedUSD Error at'+' '+str(d1)+' '+str(current_time)+' '+'\nError:'+ str(e) + str(e.args)) 
